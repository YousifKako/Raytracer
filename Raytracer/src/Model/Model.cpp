...
Model::Model(const std::string_view& model_name)
{
...
void Model::load()
{
    std::ifstream model(this->model_name, std::ios::in);
    std::string buffer = "";
    ...
    while (std::getline(model, buffer))
    {
        if (buffer[0] == 'o')
        {
            ...
            mesh->set_object_name(mesh_name);
        }
        else if (buffer[0] == 'v')
        {
            auto result = split(buffer, ' ');
            if (buffer[1] == ' ')
        ...
        {
            auto result  = split(buffer, ' ');
            auto vertex1 = split(result[1], '/');
            auto vertex2 = split(result[2], '/');
            auto vertex3 = split(result[3], '/');
            Triangle triangle = { };
